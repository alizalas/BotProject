...
async def finish_result_format(update, context, chat_id):
    ...
    page_results = results[start_idx:start_idx + PAGINATION_CNT] if start_idx + PAGINATION_CNT <= len(results) else results[start_idx:]
    message = f"стр. {current_page + 1}/{total_pages}\n\n"
    keyboard = []

    if kind == "book":
        format_str = "📌 {title}, {author}, {year} год, жанр: {genre}, ссылка: {link}\n"
        for item in page_results:
            message += format_str.format(title=item.title, author=item.author, year=item.year, genre=item.genre,
                                         link=item.link)
    else:
        format_str = "📌 {title}, {director}, {year} год, жанр: {genre}, продолжительность: {duration} мин, рейтинг: {rating}, ссылка: {link}\n"
        for item in page_results:
            message += format_str.format(title=item.title, director=item.director, year=item.year, genre=item.genre,
                                         duration=item.duration, rating=item.rating, link=item.link)
